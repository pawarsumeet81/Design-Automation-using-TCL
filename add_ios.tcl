# Title:       add_ios.tcl
#
# Description: This script is modified to insert seperate input/output pads depending on the type of port.
#
# Usage:       prompt> set add_ios 1
#     source ../scripts/recovery.tcl
#             report_timing
#     source ../scripts/add_ios.tcl
#
...
# inside each side, we are checking the direction if it is input or output.
if { [get_attribute [get_port $port ] direction ] == "in" } {

# if the direction is input, we are inserting the appropriate input pad
     create_cell $this_io saed32io_wb_ss0p95v125c_2p25v/I1025_NS
     set side 2
     }
else {

# if the direction is output, we are inserting the appropriate output pad
      create_cell $this_io saed32io_wb_ss0p95v125c_2p25v/D8I1025_NS        set side 2
      }                              
...
# if the direction is output, we are inserting the appropriate output pad
      create_cell $this_io saed32io_wb_ss0p95v125c_2p25v/D8I1025_NS      set side 4
      }        
  }
  if {  $side == "r"  } {
